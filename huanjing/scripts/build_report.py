...
def build_report(metrics, city, period, latitude, longitude):
    ...
    uncertainty_parts.append(
        "脆弱性评分基于国家统计局第七次人口普查公开数据的省级65+占比计算"
        "(非市级/社区级)，且未纳入空调可及性、绿地覆盖率等适应资源数据，"
        "权重设计参考Gasparrini 2015与Sci Adv 2025")
    ...
    # 脆弱性评分的真实来源: 国家统计局七普公开数据 (省级65+占比)
    # 权重设计(50%人口脆弱)基于文献, 但评分本身来自真实统计
    fairness = {
        "status": "已覆盖重点人群" if at_risk else "无重点人群提示",
        "covered_groups": [g["group"] for g in at_risk],
        "vulnerable_assessment": (
            f"脆弱性评分 {vuln_score}/100。"
            "评分依据: 该市所在省份65+老年人口占比(国家统计局第七次全国人口普查公开数据)"
            "相对全国均值13.5%归一化计算。"
            "局限: 仅省级数据, 未含市级/社区级老年人口分布; "
            "未纳入低收入社区分布、流动人口数据、空调可及性、绿地覆盖率。"
...
def build_word(report, out_path):
    ...
    if tmean2 is not None:
        try:
            ...
            from compute_indices import rr_from_tmean, MMT_DEFAULT
            mmt = MMT_DEFAULT
            rr = rr_from_tmean(tmean2, mmt)
        except Exception:
            rr = 1.0
            mmt = 24.0
        if rr < 1.1:
            ...
            rr_desc = "显著高于最适温度(低温/高温风险明显)"
        lines.append(f"日均温{tmean2:.1f}C。基于Gasparrini 2015 J型曲线(MMT={mmt:.0f}C)估算"
                     f"相对死亡风险RR约{rr:.2f}，{rr_desc}")
    if not lines:
        lines.append("当前气象条件温和，无显著健康风险")
    for ln in lines:
        _para(f"- {ln}", 12)

    # 五、风险演变趋势解读 (基于近15天历史+未来7天预报)
    trend = report.get("trend", {})
    if trend and trend.get("dates"):
        _heading("五、风险演变趋势")
        dates = trend["dates"]
        scores = trend.get("risk_scores", [])
        is_fc = trend.get("is_forecast", [])
        today_idx = trend.get("today_idx", len(dates) - 1)
        # 历史段最高风险
        hist_scores = [s for i, s in enumerate(scores) if i <= today_idx and s is not None]
        fut_scores = [s for i, s in enumerate(scores) if i > today_idx and s is not None]
        _para(f"评估期覆盖近15天历史与未来7天预报(共{len(dates)}天)。", 12)
        if hist_scores:
            hmax = max(hist_scores)
            # 在历史段切片内定位, 再换算全局索引 (避免跨段同名值取错)
            hmax_i = hist_scores.index(hmax)
            hmax_date = dates[hmax_i] if hmax_i < len(dates) else ""
            _para(f"历史段: 最高风险出现在{hmax_date}，评分{hmax:.2f}/5。", 12)
        if fut_scores:
            fmax = max(fut_scores)
            # 在预报段切片内定位, 再换算全局索引 (today_idx+1 为预报段起点)
            fmax_i = today_idx + 1 + fut_scores.index(fmax)
            fmax_date = dates[fmax_i] if fmax_i < len(dates) else ""
            # 当前风险 = 预报段第一天(今天); 若为空则取历史段最后一天
            cur = fut_scores[0] if fut_scores[0] is not None else (
                scores[today_idx] if today_idx < len(scores) else 0)
            _para(f"预报段: 未来最高风险预计出现在{fmax_date}，评分{fmax:.2f}/5。"
                  f"与当前风险({cur:.2f}/5)相比，"
                  f"{'风险上升，建议提前做好防护准备' if fmax > cur + 0.5 else '风险变化不大，维持常规防护'}。", 12)
        if trend.get("tmax"):
            fut_tmax = [trend["tmax"][i] for i in range(len(dates)) if i > today_idx
                        and trend["tmax"][i] is not None]
            if fut_tmax and max(fut_tmax) >= 35:
                _para(f"注意: 预报最高气温可达{max(fut_tmax):.1f}C，达到高温阈值，需提前防范高温健康风险。", 12)
        doc.add_paragraph("")
    ...
    doc.save(out_path)
    verify_word_numbers(out_path, report)


def verify_word_numbers(docx_path, report):
    """自检: 验证 Word 中的关键数字与 report.json 数据一致 (杜绝写死/错值)。"""
    from docx import Document
    m = report["metrics"]
    aq = m.get("air_quality", {})
    # 关键数字清单: (报告值, 数据源值, 描述)
    checks = [
        (str(m.get("temperature_max_c", "")), str(m.get("temperature_max_c", "")), "最高温"),
        (str(m.get("temperature_min_c", "")), str(m.get("temperature_min_c", "")), "最低温"),
        (str(m.get("temperature_mean_c", "")), str(m.get("temperature_mean_c", "")), "日均温"),
        (str(m.get("humidity_mean_pct", "")), str(m.get("humidity_mean_pct", "")), "湿度"),
        (str(m.get("heat_index_max_c", "")), str(m.get("heat_index_max_c", "")), "热指数"),
        (str(m.get("wbgt_max_c", "")), str(m.get("wbgt_max_c", "")), "WBGT"),
        (str(aq.get("aqi_value", "")), str(aq.get("aqi_value", "")), "AQI"),
        (str(m.get("vulnerability_score", "")), str(m.get("vulnerability_score", "")), "脆弱性"),
        (str(m.get("risk_score", "")), str(m.get("risk_score", "")), "风险分"),
    ]
    # 读取 Word 全文
    ...
    problems = []
    for report_val, data_val, name in checks:
        if data_val == "None" or data_val == "":
            continue
        if report_val and str(data_val) not in text and str(data_val) + " " not in text:
            problems.append(f"{name}: Word缺失值 {data_val}")
    # 检查 RR (健康影响解读中)
    tmean = m.get("temperature_mean_c")
    if tmean is not None:
        try:
            from compute_indices import rr_from_tmean
            rr = rr_from_tmean(tmean)
            rr_str = f"{rr:.2f}"
            if rr_str not in text:
                problems.append(f"RR值 {rr_str} 未出现在Word中")
        except Exception:
            pass
    if problems:
        print(f"  [警告] Word自检发现 {len(problems)} 处不一致:")
        for p in problems:
            print(f"    - {p}")
    else:
        print("  [OK] Word自检通过: 关键数字与数据一致")
...
def build_markdown(report):
    ...
    if trend and trend.get("dates"):
        ...
        scores = trend.get("risk_scores", [])
        today_idx = trend.get("today_idx", len(dates) - 1)
        lines += ["", "## 四、风险演变趋势（近15天历史+未来7天预报）", ""]
        hist_scores = scores[:today_idx + 1]
        if hist_scores:
            hmax = max(hist_scores)
            hmax_i = hist_scores.index(hmax)  # 切片内定位
            lines += [f"- 历史段最高风险评分: {hmax:.2f}/5（{dates[hmax_i]}）"]
        fut = scores[today_idx + 1:] if today_idx + 1 < len(scores) else []
        if fut:
            fmax = max(fut)
            fmax_i = today_idx + 1 + fut.index(fmax)  # 切片内定位再换算全局
            lines += [f"- 预报段最高风险评分: {fmax:.2f}/5（{dates[fmax_i]}）"]
            lines += [f"- 未来7天风险走势: {'上升' if max(fut) > max(hist_scores) else '平稳或下降'}，建议{'提前防护' if max(fut) > 2 else '常规关注'}"]
    lines += ["", "## 五、不确定性", ""]
...
def main():
    ...
    # 大屏 HTML + 静态风险图 (先于 Word/Markdown, 让趋势数据写入 report.json)
    if args.map:
        build_map(report, metrics, args, args.lat or 31.0, args.lon or 121.0,
                  args.out_dir)
        # build_map 可能更新了 report(含趋势), 重新加载
    ...
    if args.map:
        expected += ["dashboard.html", "risk_map.png", "risk_grid_points.json"]
    missing = []
    ...
    if os.path.exists(docx_path) and os.path.getsize(docx_path) > 0:
        print(f"  [OK] report.docx ({os.path.getsize(docx_path)} bytes)")
    elif not os.path.exists(docx_path):
        missing.append("report.docx")
    if missing:
        print(f"\n[错误] 以下输出缺失: {missing}")
        sys.exit(1)
    print("\n所有输出完整。")
...
def build_map(report, metrics, args, lat, lon, out_dir):
    ...
    try:
        ...
        try:
            ...
            # 趋势数据写入 report.json (供 Word 报告解读)
            report.setdefault("trend", {})
            report["trend"] = {
                "dates": rt_trend["dates"],
                "risk_scores": rt_trend["risk_scores"],
                "is_forecast": rt_trend["is_forecast"],
                "tmax": rt_trend.get("tmax", []),
                "tmin": rt_trend.get("tmin", []),
                "tmean": rt_trend.get("tmean", []),
                "today_idx": sum(1 for x in rt_trend["is_forecast"] if not x) - 1,
            }
            with open(os.path.join(out_dir, "report.json"), "w", encoding="utf-8") as _f:
