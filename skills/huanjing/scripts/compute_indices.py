...
# ==================== 温度-健康 RR 模型 (单一事实来源) ====================
# 基于 Gasparrini 2015 Lancet 的 J 型曲线近似 (全部调用方共用此函数)
# 文献校准: 冷端 -20C 时 RR≈2.6, 热端 40C 时 RR≈1.4 (对照 Gasparrini 2015 Fig.2)
# 二次指数 RR = exp(a*(T-MMT)^2):
#   冷端 a=0.0005 -> T=-20: exp(0.0005*1936)=2.63; T=-25: exp(0.0005*2401)=3.32
#   热端 a=0.0013 -> T=40: exp(0.0013*256)=1.40; T=45: exp(0.0013*441)=1.77
RR_COLD_COEF = 0.0005
RR_HOT_COEF = 0.0013
MMT_DEFAULT = 24.0  # 全球中位最适温度 (Gasparrini 2015)


def rr_from_tmean(tmean, mmt=MMT_DEFAULT):
    """由日均温估算相对死亡风险 RR (Gasparrini 2015 J 型曲线近似)。

    RR = exp(a*(T-MMT)^2), 冷端 a=0.0005, 热端 a=0.0013。
    返回范围约 1.0-3.5, 不会爆炸。
    """
    if tmean is None:
        return 1.0
    coef = RR_HOT_COEF if tmean >= mmt else RR_COLD_COEF
    return float(math.exp(coef * (tmean - mmt) ** 2))
