...
def _jcurve_svg(tmean):
    ...
    - 本 Skill 用二次指数近似: RR=exp(a*(T-MMT)^2),
      冷端 a=0.0005, 热端 a=0.0013 (对照 Gasparrini 2015 Fig.2 校准)
    - 真实曲线基于 DLNM 模型, 此处为示意性近似
    """
    from compute_indices import rr_from_tmean, MMT_DEFAULT
    topt = MMT_DEFAULT  # 24.0
    T = np.linspace(-10, 45, 150)
    # 用统一 RR 函数 (与 Word/趋势图一致)
    RR = np.array([rr_from_tmean(t, topt) for t in T])
    RR = np.clip(RR, 0, 3.5)
    W, H = 260, 130
    pad = 10
    ...
    # 标注当前日均温度
    rr_now = rr_from_tmean(tmean, topt)
...
def _field_map_html(geojson_path, points, field, stops, city_lat, city_lon, zoom=10,
                    city_name="", fallback_fields=None, dynamic=False):
    """沿行政区边界裁剪的色斑图 (folium)。

    field: 主字段; fallback_fields: 主字段数据不足时依次尝试的字段
    dynamic: True 时色阶按数据 min-max 动态拉伸 (避免窄范围同色)
    """
    import folium
    ...
    # 字段选择: 主字段有效点不足时用备选
    use_field = field
    candidates = [field] + (fallback_fields or [])
    for f2 in candidates:
        n_valid = sum(1 for p in points if p.get("risk_level") != "数据失败"
                      and p.get(f2) is not None)
        if n_valid >= 4:
            use_field = f2
            break
    else:
        return "", stops, use_field

    pts = [p for p in points if p.get("risk_level") != "数据失败"
           and p.get(use_field) is not None]
    if len(pts) < 4:
        return "", stops, use_field

    lats = np.array([p["lat"] for p in pts])
    lons = np.array([p["lon"] for p in pts])
    vals = np.array([float(p[use_field]) for p in pts])
    ...
    if dynamic:
        ...
        # 用暖色系色阶 (蓝-黄-红, 类似气象图)
        stops_dyn = [
            (vmin, "#2c7fb8"), (vmin + (vmax - vmin) * 0.33, "#74c7e3"),
            (vmin + (vmax - vmin) * 0.5, "#fee08b"),
            (vmin + (vmax - vmin) * 0.75, "#fdae61"),
            (vmax, "#d73027"),
        ]
        stops = stops_dyn
    ...
    # 初始 zoom 按跨度估算 (保守), 实际视野由 fitBounds 精确控制
    ...
    # 注入 fitBounds 脚本: 暴露 _fitCity 供父页面 tab 切换时重新调用
    # (非 active 的 iframe 容器尺寸为0, fitBounds 计算会出错, 必须等可见后执行)
    map_var = m.get_name()
    fit_js = (
        f'<script>'
        f'window._cityBounds = [[{b[1]}, {b[0]}], [{b[3]}, {b[2]}]];'
        f'window._fitCity = function(){{ try {{ {map_var}.fitBounds(window._cityBounds, {{padding:[20,20]}}); }} catch(e){{}} }};'
        f'window.addEventListener("load", function(){{ setTimeout(window._fitCity, 300); }});'
        f'</script>'
    )
    m.get_root().html.add_child(folium.Element(fit_js))
    dlat = glat[1] - glat[0]
    dlon = glon[1] - glon[0]
    for i in range(grid):
        for j in range(grid):
    ...
    return m.get_root().render(), stops, use_field
...
def build(points, era5, aqi, report, metrics, geojson_path, city, period, out_html,
    ...
    map_temp = _field_map_html(geojson_path, points, "temperature_max_c", TEMP_STOPS, city_lat, city_lon, city_name=city, dynamic=True)
    map_rh = _field_map_html(geojson_path, points, "humidity_mean_pct", RH_STOPS, city_lat, city_lon, city_name=city, dynamic=True)
    map_pm = _field_map_html(geojson_path, points, "heat_index_max_c", PM_STOPS, city_lat, city_lon, city_name=city,
                             fallback_fields=["wbgt_max_c", "temperature_max_c"], dynamic=True)
    # 解包返回值 (html, stops_used, field_used)
    map_risk, risk_stops_used, _ = map_risk
    map_temp, temp_stops_used, _ = map_temp
    map_rh, rh_stops_used, _ = map_rh
    map_pm, pm_stops_used, pm_field_used = map_pm
    ...
    def _stops_legend(stops, unit=""):
        """由 stops 生成图例 (与地图实际色阶一致)。"""
        parts = []
        for i in range(len(stops) - 1):
            v0, c0 = stops[i]
            v1, _ = stops[i + 1]
            parts.append(f'<span class="lg"><i style="background:{c0}"></i>{v0:g}-{v1:g}{unit}</span>')
        parts.append(f'<span class="lg"><i style="background:{stops[-1][1]}"></i>>{stops[-1][0]:g}{unit}</span>')
        return "".join(parts)

    temp_lg = _stops_legend(temp_stops_used, "C")
    rh_lg = _stops_legend(rh_stops_used, "%")
    pm_unit = "C" if pm_field_used != "humidity_mean_pct" else "%"
    hi_lg = _stops_legend(pm_stops_used, pm_unit)
  ...
  <div class="center">
    <div class="mapwrap">
      <div class="tabs">
        ...
        <div class="tab" data-t="3">&#x1F32B; 热指数/WBGT</div>
      </div>
...
function _switchTab(i){
  ...
  // iframe 可见后重新 fitBounds (非 active 时容器尺寸为0, fitBounds 计算会出错)
  var _fr = document.querySelectorAll('.tabpane iframe')[i];
  if (_fr && _fr.contentWindow && _fr.contentWindow._fitCity) {
    setTimeout(function(){ _fr.contentWindow._fitCity(); }, 150);
  }
  _tabIdx=i;
}
document.querySelectorAll('.tab').forEach(function(el){
  el.addEventListener('click',function(){
    clearInterval(_tabTimer);
    _switchTab(parseInt(el.getAttribute('data-t')));
    _tabTimer=setInterval(function(){_switchTab((_tabIdx+1)%4);},5000);
  });
});
_tabTimer=setInterval(function(){_switchTab((_tabIdx+1)%4);},5000);
</script>
